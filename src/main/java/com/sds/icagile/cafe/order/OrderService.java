...
import com.sds.icagile.cafe.payment.*;
...
public class OrderService {
    ...
    public Order create(int customerId, List<Map<String, Object>> orderItemList, int payment) {
        ...
        double totalCost = getTotalCost(orderItemList, order, orderItems);

        // 2. 매월 마지막 날이면 10% 할인
        double discountedTotalCost = this.getDiscountedTotalCost(totalCost);
    ...
    private double getTotalCost(List<Map<String, Object>> orderItemList, Order order, List<OrderItem> orderItems) {
        double totalCost = 0;
